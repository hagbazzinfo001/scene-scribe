...
export default function ProjectWorkspace() {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data,
        error
      } = await supabase.from('user_assets').select('*').eq('project_id', projectId).order('created_at', {
        ascending: false
      });
      if (error) throw error;
      return data;
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data,
        error
      } = await supabase.from('jobs').select('*').eq('project_id', projectId).order('created_at', {
        ascending: false
      });
      if (error) throw error;
      return data;
  ...
  useEffect(() => {
    if (!projectId) return;
    const channel = supabase.channel('project-updates').on('postgres_changes', {
      event: '*',
      schema: 'public',
      table: 'user_assets',
  ...
  const handleInvite = async () => {
    try {
      ...
      toast({
        title: 'Copy failed',
        description: 'Unable to copy link, opening instead.'
      });
  ...
  return <div className="flex-1 flex h-screen">
      {/* Main Content */}
        ...
        <div className="border-b p-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                ...
                <Button variant="destructive" size="sm" onClick={handleDeleteProject} className="text-slate-50 bg-[#121312]/[0.31]">
                  Delete Project
                </Button>
              </div>
            </div>
          ...
          <Tabs value={selectedTab} onValueChange={setSelectedTab} className="h-full flex flex-col">
            ...
            <TabsContent value="assets" className="flex-1">
              <div className="space-y-6">
                <ImportAssetDropzone projectId={projectId!} onAssetUploaded={handleAssetUploaded} />
                ...
                {assets.length > 0 && <Card>
                    ...
                    <CardContent>
                      <div className="space-y-3">
                        {assets.map((asset: any) => <div key={asset.id} className="flex items-center justify-between p-3 border rounded-lg">
                            ...
                            <div className="flex items-center gap-2">
                              <Button variant="ghost" size="sm" onClick={() => handleDeleteAsset(asset.id)} className="text-neutral-50">
                                {t('delete')}
                              </Button>
                              {asset.processing_status === 'completed' && <Badge variant="default">Ready</Badge>}
                              {asset.processing_status === 'processing' && <Badge variant="secondary">Processing</Badge>}
                              {asset.processing_status === 'failed' && <Badge variant="destructive">Failed</Badge>}
                            </div>
                          </div>)}
                      </div>
                    </CardContent>
            ...
            <TabsContent value="breakdown" className="flex-1">
              <div className="mb-4">
                ...
                try {
                  const {
                    data,
                    error
                  } = await supabase.functions.invoke('trigger-jobs');
                  if (error) throw error;
                  toast({
              ...
              <BreakdownResults jobs={jobs} onExport={(job, format) => {
              if (format === 'csv' && job.output_data?.scenes) {
                const csvData = [['Scene', 'Location', 'Description', 'Characters'], ...job.output_data.scenes.map((scene: any) => [scene.id || '', scene.location || '', scene.description || '', (scene.characters || []).join(', ')])];
            ...
            <TabsContent value="vfx" className="flex-1">
              <div className="space-y-6">
                ...
                {/* Job Results */}
                {jobs.filter(job => ['mesh-generator'].includes(job.type)).length > 0 && <div className="space-y-4">
                    <h3 className="text-lg font-semibold">Generated Meshes</h3>
                    {jobs.filter(job => ['mesh-generator'].includes(job.type)).map(job => <JobPreview key={job.id} job={job} onDownload={handleJobDownload} />)}
                  </div>}
              </div>
            </TabsContent>
